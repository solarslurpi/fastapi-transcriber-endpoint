...
async def transcribe_mp3(mp3_filepath: str, logger: LoggerBase):
    ...
    yield {'transcription_time': transcription_time}

# For use with more than one chapter, define a function to slice audio
def slice_audio(mp3_filepath: str, start_ms: int, end_ms: int) -> str:
...
async def transcribe_chapters(chapters: list, logger: LoggerBase, mp3_filepath: str, hf_model_name: str = "distil-whisper/distil-large-v3", compute_type_pytorch: torch.dtype = torch.float16):
    for chapter in chapters:
        ...
        mp3_path = slice_audio(mp3_filepath, start_ms, end_ms) if end_ms else mp3_filepath
